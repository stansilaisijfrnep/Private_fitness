...
1. `profile/PROFILE.md` — who the athlete is, stats, injuries, goals, constraints.
   ...
   `profile/schedule.md` — the class timetable (in person vs online) and the free training window
   per weekday, generated by `tools/schedule.py` from `data/schedule/`. Use it to place sessions,
   meals and sleep advice around classes. If it says no classes found, ask for a fresh
   Calendar.app export (`data/schedule/README.md`).
