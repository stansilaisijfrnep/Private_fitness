# Athlete profile

Intake completed 2026-09-05 via interview. Update this file whenever something changes.

## Basics
| Field | Value |
|---|---|
| Age | 22 |
| Sex | Male (assumed for calorie math, not asked explicitly; correct if wrong) |
| Height | 194 cm |
| Bodyweight (start) | 78.65 kg evening reading 2026-09-05 on a new scale. Morning baseline from 2026-09-06. The 82 kg from the intake was a guess. |
| Waist at navel (start) | not yet measured |
| Estimated body fat | roughly 11 to 14 %. Visible muscle, lean, tall frame. BMI 20.9 |
| Date of intake | 2026-09-05 |
| Language | Writes English and sometimes German. Answer in whichever they use. |
| Location | France, inferred from Carrefour products and French gym signage. Food estimates use French portions and products. |

## Training history
- Years training consistently: 3+, mostly consistent.
- Current split: push / pull / legs, but legs are basically skipped.
- Sessions per week possible: 5.
- Session length available: 60 to 75 minutes.
- Gym: full commercial gym, Fitness Park style (Technogym, Hammer Strength, Eleiko). Equipment inventory in `profile/gym.md`.
- Other sport: boxing, padel, sometimes basketball. Load currently uncertain; gym is the focus for now.
- Coach's assessment: **intermediate by years, early-intermediate by results.**
  Pull-ups are strong. Bench is far below what 3 years should give. Legs are untrained.
  The pressing pattern was never progressively overloaded ("depends on the day"), and food
  was never sufficient. The pull-up number proves the muscle-building machinery works.

## Current strength (start of block 1)
| Lift | Load x reps | RIR | Date | Note |
|---|---|---|---|---|
| Flat barbell bench | 55 kg x 10 to 15 | ? | 2026-09 | inconsistent, not logged, varies by day |
| Strict pull-up | bodyweight x 9 to 15 | ? | 2026-09 | strong point |
| Back squat | none | | | never trained |
| Deadlift | none | | | never trained |
| Overhead press | unknown | | | test in week 1 |
| Row | unknown | | | test in week 1 |

Best lifts ever: not known. Week 1 of block 1 is a test week; fill this table from the logs.

## Injuries and limitations
| Area | Current or past | What triggers it | What is safe | Notes |
|---|---|---|---|---|
| Knee (ACL reconstruction) | past, 5 to 6 years ago | nothing currently | everything, built up progressively | fully rehabbed. Single-leg strength, hamstring strength and landing mechanics stay in the program for life. No max-effort plyometrics in block 1. |
| Lower and upper back | current, mild | long sitting (remote desk job + studying) | all gym work, no pain in training | postural. Fix: hinge and glute strength, rear delt and upper back volume, stand up every 45 min, daily 2 min dead hang and hip flexor stretch. If it ever appears during lifting, stop the set and report it. |

Mobility restrictions: none reported. Assess squat depth in week 1.
Medical conditions or medication: none.

## Self-assessment and coach's read
- Strong points: back and pull-ups. Athletic base from sport.
- Weak points: chest and pressing (bench 55 kg after 3 years), legs (skipped), consistency of loading.
- Body parts to prioritise: whole upper body, abs, legs for muscle plus knee strength and injury prevention.
- What failed before and why: 3 years of training with no leg work, no logged progression, and
  under-eating. Effort was there. Structure and food were not.

## Nutrition
- Tracks now: no. Did before, stopped.
- Typical structure: 3 meals plus snacks. Gets full fast. Forgets to eat when busy.
- Restrictions: none, eats everything.
- Cooking: cooks and orders.
- Alcohol: avoids during the week. When going out at weekends, drinks a lot. Intends to go out
  less over the next 12 months. Athlete stated this is their own choice and not the coach's
  responsibility. Coach's stance: noted and respected. It gets logged like a meal, and the data
  will speak for itself at check-ins. No nagging.
- Caffeine: not discussed. Rule: none after 15:00.

## Recovery and lifestyle
- Sleep: 6 to 7 hours. Below target. Fixed wake time and 7.5 h minimum are the assignment.
- Job: remote desk job plus studying. Low daily burn, lots of sitting.
- Steps: 5,000 to 8,000.
- Supplements: creatine already. Keep at 5 g daily.

## Goals
- Primary: visibly bigger, muscle size. Upper body and abs. Legs for muscle, strong knees and
  injury prevention for boxing, padel, basketball.
- Timeframe: 12 months.
- Priority order: size > strength > leanness. Athleticism maintained through leg and single-leg work.
- Realistic 12-month outcome if executed: 86 to 89 kg on the scale with waist no more than
  3 cm above start. That is roughly 7 to 9 kg of lean mass. Bench in the 90 to 100 kg range for reps.

## Targets (set 2026-09-05 at intake)
| Target | Value | Reason |
|---|---|---|
| Calories per day | 3300 kcal | Mifflin-St Jeor at 78.65 kg gives BMR ~1890, activity factor 1.6 gives ~3030 maintenance, plus ~270. Unchanged after the weight correction. Recalibrate after 2 weeks of scale data. |
| Protein per day | 170 g | ~2.2 g per kg at 78.65 kg. Kept, more protein is not a problem. Spread over 4 to 5 feedings of 35 to 45 g. |
| Fat per day | 80 g | ~1.0 g per kg. |
| Carbs per day | 475 g | The rest. Training fuel. |
| Bodyweight gain per week | 0.2 to 0.3 kg | ~0.3 to 0.4 % of bodyweight. Faster is fat. |
| Eating occasions | 5 fixed times | 08:00, 11:30, 15:00, 18:30, 21:30. Alarms on. Two can be shakes. |
| Steps per day | 7,000 minimum | Health and appetite. Not more than 10,000, it eats calories. |
| Sleep | 7.5 h minimum, fixed wake time | Non-negotiable for muscle gain. |
| Sessions per week | 5 | See program/current.md |
| Creatine | 5 g daily | Already taking. Keep. |
| Waist | measure every Sunday morning | The fat-gain alarm. More than +1 cm in 2 weeks means calories come down 150. |

## Calorie adjustment rules
- Weigh every morning after the bathroom, before food. Only the 7-day average counts.
- Week 1 and 2 are calibration. Eat 3300, log everything, and we see what the scale does.
- Average gain under 0.2 kg per week over 2 weeks: add 200 kcal (carbs).
- Average gain over 0.5 kg per week over 2 weeks: remove 150 kcal.
- Waist up more than 1 cm in 2 weeks with weight on target: remove 150 kcal, check alcohol log.

## The hard-gainer eating rules (for someone who gets full fast and forgets to eat)
1. Fixed meal times with alarms. Hunger is not the trigger, the clock is.
2. Calorie density first: white rice, pasta, bread, olive oil, nuts, nut butter, whole milk,
   cheese, eggs, fatty fish, dried fruit. Do not fill up on salad and water before the calories are in.
3. The daily shake: 500 ml whole milk, 80 g oats, 1 banana, 30 g whey, 30 g peanut butter.
   About 950 kcal and 55 g protein. Split in two if it is too much at once.
4. Every meal has 35 to 45 g protein. Meat, fish, eggs, dairy, whey.
5. Ordering defaults: rice bowl with double meat and double rice; kebab plate with rice; sushi with
   extra rice; pasta with meat. Pizza is fine, log it.
6. Carbs 1 to 2 hours before training. Shake after training if the next meal is more than 2 hours away.
7. Night out: 2 litres of water and a protein meal before bed. Next day is a normal eating and
   training day. Never "make up" for it by skipping food.
8. Single-sitting ceiling: dense solid food (rice, pasta, meat) tops out around 300 to 350 kcal
   before feeling stuffed, even with more sitting in front of them (confirmed 2026-09-05, a 330 kcal
   tuna-rice burrito could not be finished). Plan solid meals in that range, not larger. To hit a
   bigger total, add a second sitting or a shake, do not make one plate bigger. Batch-cooked meals
   should be portioned into 300 to 350 kcal servings from the start, not split into fewer, bigger ones.
